...
public class AdminDaoImpl  implements AdminDao {
       ...
       List<Msg> msgs=null;
        ...
        for (Msg msg:msgs){
            try {
                ...
                goods=runner.query("select * from goods where id=?",new BeanHandler<Goods>(Goods.class),msg.getGoodsId());
                System.out.println(msg.getGoodsId());
                user=runner.query("select * from user where id=?",new BeanHandler<User>(User.class),msg.getUserid());
    ...
    public List<Msg> repliedMsg() {
        ...
        for (int i=0;i<msgs.size();i++){
            try {
                User user=new User();
                msgs.get(i).setGoods(runner.query("select name from goods where id=?",new BeanHandler<Goods>(Goods.class),msgs.get(i).getGoodsId()));
                user=runner.query("select nickname from user where id=?",new BeanHandler<User>(User.class),msgs.get(i).getUserid());
                user.setName(user.getNickname());
